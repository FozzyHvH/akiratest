	...
	do
		...
		function Library:Window(Options)
			local Base = {
				...
				Title = Options.Name or Options.Name or Options.Name or "vanity";
			};
			...
			version.Text = "<font color=\"#4e4e4e\">version: </font> developer"
...
do
    ...
    do
        local Config = Pages["Settings"]:Section({Name = "Config", LeftTitle = "Config", RightTitle = "Lua Addons"}) do
            ...
            local LUAList = Config:List({Name = "Lua Addons", Flag = "LuaAddonList", Options = {}, Side = "Right"})
            local loadedluas = {}
            local abc = false
            local flags = Library.Flags
            Config:Textbox({Flag = "SettingsConfigurationName", Placeholder = "Config name"})
    
            if not isfolder("configtest") then 	
                makefolder("configtest")
            end 
            if not isfolder("configtest/Configs") then 
                makefolder("configtest/Configs")
            end 

            if not isfolder("configtest/LuaAddons") then
                makefolder("configtest/LuaAddons")
            end
    
            local function UpdateConfigList()
                local files = listfiles("configtest/Configs")
                local configs = {}
                
                for _, file in pairs(files) do
                    if file:sub(-4) == '.cfg' then
                        local name = string.gsub(string.gsub(file, "configtest/Configs\\", ""), ".cfg", "")
            ...
            local function UpdateLuaList()
                local files = listfiles("configtest/LuaAddons")
                local luas = {}
                for _, file in pairs(files) do
                    if file:sub(-4) == '.lua' then
                        local name = string.gsub(string.gsub(file, "configtest/LuaAddons\\", ""), ".lua", "")
            ...
            Config:Button({Name = "Create", Callback = function()
                ...
                if name and name ~= "" then
                    ...
                    writefile("configtest/Configs/" .. name .. ".cfg", config)
                    UpdateConfigList()
                end
            end})
            ...
            Config:Button({Name = "Load", Callback = function()
                local name = flags["SettingConfigurationList"]
                if name and isfile("configtest/Configs/" .. name .. ".cfg") then
                    local config = readfile("configtest/Configs/" .. name .. ".cfg")
            ...
            Config:Button({Name = "Delete", Callback = function()
                local name = flags["SettingConfigurationList"]
                if name then
                    if isfile("configtest/Configs/" .. name .. ".cfg") then
                        delfile("configtest/Configs/" .. name .. ".cfg")
                    end
                    UpdateConfigList()
                end
            end})
            ...
            Config:Colorpicker({
                Name = "Menu Accent", 
                Flag = "MenuAccent", 
                Default = Color3.new(1, 0.349019, 0.490196), 
            ...
            Config:Button({Name = "Load Lua", Side = "Right", Callback = function()
                local name = flags["LuaAddonList"]
                name = name:gsub("%.lua$", "")
                name = name:match("[^/\\]+$") or name
                
                if name and isfile("configtest/LuaAddons/" .. name .. ".lua") then
                    if loadedluas[name] and loadedluas[name]._unload then
                        pcall(loadedluas[name]._unload)
                        loadedluas[name] = nil
                    end
                    
                    local src = readfile("configtest/LuaAddons/" .. name .. ".lua")
            ...
            Config:Button({Name = "Unload Lua", Side = "Right", Callback = function()
                local name = flags["LuaAddonList"]
            ...
            Config:Button({Name = "Refresh", Side = "Right", Callback = function()
                UpdateLuaList()
            end})
