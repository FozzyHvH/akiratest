...
do 
	...
	do 
		...
		do 
			...
			utility.drawings["Sub"] = utility:drawObject("Text", {Outline = true, Text = "akira", Color = Color3.new(1, 1, 1)})
			utility.drawings["Domain"] = utility:drawObject("Text", {Outline = true, Text = ".lua", Color = Color3.fromRGB(132, 108, 188)})
			--
			...
			do 
				Messages = {
					...
					["Akira"] = {
						"Owned By Akira",
						"Akira Akira Akira RAHHHHH",
						"Slammed by Akira",
						"YOU GOT SLAMMED BY Akira",
						"SEEMS LIKE YOU NEED Akira GG/Akira",
						"GG/Akira GG/Akira GG/Akira",
						"GG/Akira <-- THIS WILL LET YOU COPE WITH YOUR ISSUES",
						"WHAT YOU CANT BEAT Akira?",
						"PRO TIP: BUY Akira",
					},
				}
				utx = {
...
local Window = Library:Window({Size = UDim2.new(0,580,0,600)}) do 
	...
	local misc = Window:Page({Name = "Misc", Weapons = true}) do 
		...
		local chatSpam = Movement:Section({Name = "Chat Spam", Size = 330, Side = "Left"}) do
			...
			chatSpam:Dropdown({Name = "Type", Flag = "Chat Spam Type", Options = {"Akira", "Scottish"}, Default = "Akira"})
	...
	local Config = Window:Page({Name = "Settings"}) do
		...
		--
		local CurrentList = {};
		if not isfolder("Akiravip") then 
			makefolder("Akiravip")
		end 
		if not isfolder("Akiravip/Configs") then 
			makefolder("Akiravip/Configs")
		end 
		local function UpdateConfigList()
			local List = {};
			for idx, file in ipairs(listfiles("Akiravip/Configs")) do
				local FileName = file:gsub("Akiravip/Configs\\", ""):gsub(".cfg", "")
		...
		Cfgs:Button({Name = "Create", Callback = function()
			...
			if ConfigName == "" or isfile("Akiravip/Configs/" .. ConfigName .. ".cfg") then
				return;
			end;
			writefile("Akiravip/Configs/" .. ConfigName .. ".cfg", Library:GetConfig());
			UpdateConfigList();
		end})
		Cfgs:Button({Name = "Save", Callback = function()
			local SelectedConfig = flags.SettingConfigurationList;
			if SelectedConfig then
				writefile("Akiravip/Configs/" .. SelectedConfig .. ".cfg", Library:GetConfig())
			end;
		end})
		Cfgs:Button({Name = "Load", Callback = function()
			local SelectedConfig = flags.SettingConfigurationList;
			if SelectedConfig then
				Library:LoadConfig(readfile("Akiravip/Configs/" .. SelectedConfig .. ".cfg"))
			end
		end})
		Cfgs:Button({Name = "Delete", Callback = function()
			local SelectedConfig = flags.SettingConfigurationList;
			if SelectedConfig then
				delfile("Akiravip/Configs/" .. SelectedConfig .. ".cfg")
			end
			UpdateConfigList();
		end})
...
Library:Notification("Loaded Akira.lua in " .. Floor((tick() - loadingTime) * 1000)  .."ms", 3, Library.Accent, flags["Notification Position"])
